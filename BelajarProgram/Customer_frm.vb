Public Class Customer_frm
        ...
        Else 'save
            ...
            Else
                If lblid.Text = "" Then 'simpan tambah
                    ...
                    sqLstr = "select * from mscust where customer=" &
                    ...
                    select @max=MAX(idcst) from mscust
                    DBCC CHECKIDENT(mscust,reseed,@max) "
                        Else
                            sqLstr = "DBCC CHECKIDENT(mscust,reseed,0) "
                        End If
                        sqLstr = sqLstr + "Insert into mscust(customer)values(" &
                            Aphostrophe(TextBox1.Text) & ")"
                ...
                Else 'update data
                    'cek user
                    sqLstr = "select * from mscust where customer=" &
                        Aphostrophe(TextBox1.Text) & " and customer<>" &
                        Aphostrophe(nama)
                    tabel = proses.executequery(sqLstr)
                    If tabel.Rows.Count > 0 Then 'jika ada
                        MsgBox("Jenis sudah ada")
                    Else 'update data
                        sqLstr = "update mscust set customer=" &
                            Aphostrophe(TextBox1.Text) & "where idcst=" &
                            lblid.Text
                        proses.executenonquery(sqLstr)
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        If dgv1.Rows.Count > 0 Then
            ...
            If tny = vbYes Then
                sqLstr = "Delete from mscust where idcst=" & lblid.Text
                proses.executenonquery(sqLstr)
