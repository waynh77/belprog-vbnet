Public Class Supplier_frm
        ...
        Else 'save
            ...
            Else
                If lblid.Text = "" Then 'simpan tambah
                    ...
                    sqLstr = "select * from msspl where supplier=" &
                        Aphostrophe(TextBox1.Text)
                    tabel = proses.executequery(sqLstr)
                    If tabel.Rows.Count > 0 Then
                        MsgBox("Jenis sudah ada")
                    Else 'simpan tambah
                        sqLstr = "select top(1)* from msspl"
                        tabel = proses.executequery(sqLstr)
                        If tabel.Rows.Count > 0 Then
                            sqLstr = "declare @max int;
                    select @max=MAX(idspl) from msspl
                    DBCC CHECKIDENT(msspl,reseed,@max) "
                        Else
                            sqLstr = "DBCC CHECKIDENT(msspl,reseed,0) "
                        End If
                        sqLstr = sqLstr + "Insert into msspl(supplier)values(" &
                            Aphostrophe(TextBox1.Text) & ")"
                ...
                Else 'update data
                    'cek user
                    sqLstr = "select * from msspl where supplier=" &
                        Aphostrophe(TextBox1.Text) & " and supplier<>" &
                        Aphostrophe(nama)
                    tabel = proses.executequery(sqLstr)
                    If tabel.Rows.Count > 0 Then 'jika ada
                        MsgBox("Jenis sudah ada")
                    Else 'update data
                        sqLstr = "update msspl set supplier=" &
                            Aphostrophe(TextBox1.Text) & "where idspl=" &
                            lblid.Text
                        proses.executenonquery(sqLstr)
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        If dgv1.Rows.Count > 0 Then
            ...
            If tny = vbYes Then
                sqLstr = "Delete from msspl where idspl=" & lblid.Text
                proses.executenonquery(sqLstr)
